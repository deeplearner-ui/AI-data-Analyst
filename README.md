...
- 结构化清洗操作及不可变版本链。
- t/Welch、配对检验、Mann–Whitney、Wilcoxon、卡方、Fisher、ANOVA、Kruskal、相关分析。
- OLS、逻辑回归、PCA、K-Means、层次聚类和基础时间序列分解。
- Plotly 交互图表、受限 Python 编辑器、十章证据报告、PDF/HTML 导出与可复现清单。
- OpenAI 兼容分析规划；模型只收到 schema、摘要和可关闭的脱敏样例。
...
自动 Python 执行使用 AST 许可策略、隔离子进程、过滤后的环境、项目工作目录和超时。它用于降低模型生成代码的误操作风险，不是针对恶意代码的虚拟机级隔离。数据库凭据不会写入项目；外部数据库事务提交后不能由应用自动撤销。
